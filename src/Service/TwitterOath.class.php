...
/** * Twitter-API-PHP : Simple PHP wrapper for the v1.1 API * * PHP version 5.3.10 * * 
 * @category Awesomeness * @package Twitter-API-PHP * @author James Mallison <me@j7mbo.co.uk> 
 * * @license http://opensource.org/licenses/gpl-license.php GNU Public License 
 * * @link http://github.com/j7mbo/twitter-api-php */
class TwitterOath {

    private $oauth_access_token = "320650214-nH3o3BjVYfTP9HfSWYmFkBUcoyC5HwQ2qUhz8yAz";
    private $oauth_access_token_secret = "8JMlfd5bsd7LrFhHvityFmtUCGiizUvNjeVUqcDpw";
    private $consumer_key = "YguWLyI3fltBiq1VSqfQQ";
    private $consumer_secret = "CjpZhinCu6jRxc0wTGwIJyGko3NxO1QDtHZEZFl6EE";
    private $postfields;
    private $getfield;
    private $apiUrl = 'https://api.twitter.com/1.1/';
    private $userId = '320650214'; //this is the user id        
    protected $oauth;
    public $url;
...
//Buld url        
    ...
    public function postTweetOnTwitter($sTweetMessage) {
        if (!$sTweetMessage) {
            $_SESSION['statusUpdate'] = 'Tweet is empty!';
            return false;
        } else {
            $sTweetMessage = stripslashes($sTweetMessage);
        }
//First check if there are URLS in the string        
        $aUrls = getUrlsFromString($sTweetMessage);
        if ($aUrls) {
            //Replace urls with google short url            

            foreach ($aUrls AS $sLongUrl) {
                $sTweetMessage = str_replace($sLongUrl, shortenUrl($sLongUrl), $sTweetMessage);
            }
        }
        if (strlen($sTweetMessage) > 140) {
            $_SESSION['statusUpdate'] = 'Tweet message exceeded size limit of 140 characters!';
            return false;
        }
        /** POST fields required by the URL above. See relevant docs as above * */
        $postfields = array('status' => $sTweetMessage,);
    ...
    public function deleteTweetOnTwitter($iTweetId) {
        if (!$iTweetId)
            return false;
        /** POST fields required by the URL above. See relevant docs as above * */
        $postfields = array('trim_user' => true,);
        /** Perform a POST request and echo the response * */
